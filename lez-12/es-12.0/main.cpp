...
int main()
{
    ...
    TH1F theta_0{"theta_0", "theta_0", 100, 1.569, 1.573};
    TH1F theta_1{"theta_1", "theta_1", 100, 2.545, 2.555};
    TH1F theta_2{"theta_2", "theta_2", 100, 2.650, 2.660};
    std::vector<double> theta_0_v, theta_1_v, theta_2_v;

    TH1F delta_1{"delta_1", "delta_1", 100, 0.977, 0.980};
    TH1F delta_2{"delta_2", "delta_2", 100, 1.084, 1.088};
    TH2F delta{"delta", "delta", 100, 0.977, 0.980, 100, 1.084, 1.088};
    std::vector<double> delta_1_v, delta_2_v, delta_v;

    TH1F n_1{"n_1", "n_1", 100, 1.696, 1.698};
    TH1F n_2{"n_2", "n_2", 100, 1.750, 1.752};
    TH2F n{"n", "n", 100, 1.696, 1.698, 100, 1.750, 1.752};
    std::vector<double> n_1_v, n_2_v, n_v;

    TH1F A{"A", "A", 100, 2.695, 2.705};
    TH1F B{"B", "B", 100, 59000, 61000};
    TH2F AB{"AB", "deltAB", 100, 2.695, 2.705, 100, 59600, 61000};
    ...
    for (int i{}; i < NUMBER; i++)
    {
        ...
        n_1.Fill(experiment.get_n_1_m());
        n_2.Fill(experiment.get_n_2_m());
        ...
        A.Fill(experiment.get_A_m());
        B.Fill(experiment.get_B_m());
