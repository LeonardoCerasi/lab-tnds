...
double mc_int_mean::integral(const std::function<double(double)> func, double a, double b, double f_max, int N, double precision)
{
    if (N < 0) { throw std::invalid_argument("mc_int_mean::integral : number of points must be positive."); }
    if (precision < 0) { throw std::invalid_argument("mc_int_mean::integral : precision must be positive."); }
