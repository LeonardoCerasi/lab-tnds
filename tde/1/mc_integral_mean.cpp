...
std::vector<double> mc_int_mean::integrate(const std::function<double(double)> func, double a, double b, int N, int number)
{
    std::vector<double> values;

    for (int i{}; i < number; i++)
    {
        values.push_back(integral(func, a, b, N));
    }

    return {mean(values), std_dev(values)};
}
